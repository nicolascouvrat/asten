...
inline std::ostream& operator<< (std::ostream& o, const CPUStateData& d) {
  return o
    << std::left
    << hex((uint16_t)d.pc)
    << "PREV_OP: " << instructionNames[d.latestInstruction] << " "
    << "A: " << hex((uint8_t)d.A)
    << "X: " << hex((uint8_t)d.X)
    << "Y: " << hex((uint8_t)d.Y)
...
struct NESCartridgeData {
  int prgRomUnit, chrRomUnit;
  int mirrorId, mapperId;
};
